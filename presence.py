...
class rich_presence:
	...
	def vbox_to_dict(self):
		...
		for i in range(len(self.machines)):
			...
			self.vd_machine_dict[self.vd_name]["name"] = self.vd_name
			...
			if self.config["Rich Presence"]["debug"] == "true": print(f"State of {i}: {self.machines[i].state}")
	...
	def vdict_to_formatdict(self, vf_input):
		...
		if self.vf_active_vm:
			...
			self.format_dict["vm_name"] = vf_input[self.vf_active_vm]["name"]
			self.format_dict["vbox_version"] = self.vbox.version
